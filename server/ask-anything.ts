...
import { db } from "./db";
import { sendAndAuditQuestionEmail, sendTeamNotificationEmail } from "./email";
...
export function registerAskAnythingRoutes(app: Express): void {
  app.post("/api/questions", async (req, res) => {
    ...
    let input;
    try {
      input = insertQuestionSchema.parse({
    ...
    // Database + Airtable + submitter confirmation. Can currently fail
    // (Neon on port 5432 is blocked on this host) without blocking the
    // team notification below — this becomes fully live again once the
    // WebSocket driver fix lands, no further changes needed here.
    if (db) {
      try {
        ...
        await sendAndAuditQuestionEmail(question.id, "confirmation", {
          ...
          text: "Thanks for reaching out — we've received your question and will get back to you personally as soon as we can.\n\nYour question:\n" + question.questionText,
        });
      } catch (error) {
        console.error("[ask] failed to save question to database:", error);
      }
    } else {
      console.warn("[ask] DATABASE_URL is not configured, skipping database save");
    }

    // Always notify the team directly, regardless of the database outcome.
    const notified = await sendTeamNotificationEmail({
